...
//Activities methods
    public function getActivities($keyboard) {
        ...
        // Input validations
        if (!empty($keyboard)) {
            $sql = mysql_query("SELECT  id AS id_actities ,   name  ,   logo  ,   details  ,   keyboard   FROM activities WHERE keyboard=$keyboard", $this->db);
            if (mysql_num_rows($sql) > 0) {
                $result = mysql_fetch_array($sql, MYSQL_ASSOC);
	...
	public function setActFiles($actFiles, $idActivite){
		...
		$extension=$paths[1];
		//print_r($paths);
		$sql = sprintf("INSERT INTO `dbkeaboard`.`photos` (`path`, `id_activity`, `id_product`, `type_activity`, `extension`)VALUES ('%s', $idActivite, NULL, NULL, '%s' )", 
					mysql_real_escape_string($path),mysql_real_escape_string($extension));
